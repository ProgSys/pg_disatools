	...
	ui(new Ui::MainWindow)
{
	...
	setWindowTitle(TITLE_FileManager);
	...
	if (openDialog.exec()) {
		...
		if (fileNames.size() > 0) {
			...
			if (emit openFile(fileNames[0])) {
				...
				QString title(TITLE_FileManager);
				title.append(" - ");
...
void MainWindow::treeSelectionChanged(const QItemSelection& sel, const  QItemSelection& desel) {
	...
	if (!itemName.isEmpty() &&
		(itemName.endsWith(".tx2", Qt::CaseInsensitive) || itemName.endsWith(".txp", Qt::CaseInsensitive) || itemName.endsWith(".sh", Qt::CaseInsensitive))) {
...
#else
					QString file = "\"" + QString(TITLE_SpriteSheetEditor) + ".exe\" \"" + temp->fileName() + "\"";
#endif
					setEnabled(false);
...
#else
					QString file = "\"" + QString(TITLE_DATAEditor) + ".exe\" \"" + temp->fileName() + "\"";
#endif
					setEnabled(false);
					QApplication::processEvents();
					QProcess::execute(file);
...
void MainWindow::saveSelectedImage() {
	...
	if (!imageFileName.endsWith(".TX2", Qt::CaseInsensitive) && !imageFileName.endsWith(".SH", Qt::CaseInsensitive)) 
		return;

	imageFileName = QFileInfo(imageFileName).baseName();
