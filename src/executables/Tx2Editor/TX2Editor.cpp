...
TX2Editor::TX2Editor(QWidget *parent):
	...
	setWindowTitle(TITLE_TX2Editor);
...
void TX2Editor::convertTo(){
	...
	if(dialog.isAccepted()){
		...
		else if (dialog.getResultIndex() == 7) {
			emit convertTo(PG::FILE::tx2Type::FONT);
		}else
...
void TX2Editor::updateInfo(){
	switch (m_model->image->header.type) {
		...
		case PG::FILE::tx2Type::BGRA:
				value_type->setText(m_model->image->header.colortableSize == 16? "FONT (BGRA 16 colors)" : "BGRA");
			break;
